...
- Если чат отредактирован, обрезан, свайпнут или пополнен вне ядра, расхождение больше не блокирует генерацию: недостающие реплики дописываются в историю ядра, а в панели появляется уведомление о синхронизации. Удалять реплики ядро не может — на них ссылается неизменяемый ledger.
