...
- Подтверждения дельты доступны прямо в блоке `Pending state changes`: каждое изменение описано по-человечески («Injury knee — bleeding», «Location: the flooded underpass»), у высокорисковых записей подсветка и цитата-основание, у каждой — кнопки Accept и Reject.
- Блок `What the kernel decided` показывает решения ядра: цель сцены, какие модули включились и почему, что применено к состоянию, какие находки дал критик.
...
- Critic получает не всё состояние, а релевантный срез: рамку сцены плюс факты, отношения, ресурсы, травмы и открытые линии, которые реально затронуты постом и планом. Далёкие детали больше не отвлекают проверку.
- Профиль голоса снимается с карточки детерминированно (стиль, длина фраз, плотность диалога, сокращения) и попадает в render- и repair-промпты без дополнительных вызовов к провайдеру.
- Repair правит только абзацы, на которые указывают находки. Если модель переписала остальной текст, исходный пост сохраняется, а попытка отклоняется с `repair_scope_violation`.
